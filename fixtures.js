...
fixtureMiss=function(r){
  return fxList().reduce(function(n,it){return n+fxShort(r,it.id);},0);
};
viewFixtures=function(){
  ...
  return "<h1>Fixture expected by room type</h1><div class=ok>Each item has a separate expected number for each room type.</div>"+rows+"<div class=card><h3>Add fixture</h3><input id=fxname placeholder='e.g. Wine glass'><input id=fxpar type=number min=0 value=1 placeholder='Default expected'><button class=btn id=addFx>Save fixture</button></div>";
};
