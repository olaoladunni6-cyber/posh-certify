...
function locked(r){if(!r)return false;return r.status==="certified"||r.status==="ooo";}
function sellable(r){return !!(r&&r.status==="certified"&&!r.ooo);}
window.poshSellable=sellable;
window.poshRoomLocked=locked;
window.poshHkReady=function(r){if(!r||locked(r))return false;return !!(r.laundryChecked||r.checklistDone)&&!!r.videoReady;};
function hideExtraMedia(){
  try{
    var css=document.getElementById("lockGateCss");
    if(!css){css=document.createElement("style");css.id="lockGateCss";document.head.appendChild(css);}
    css.textContent="input[type=file][accept*='image'],label.file,button#send,button#sendNow,.photoBtn,.addPhoto{display:none!important}";
    [].slice.call(document.querySelectorAll("input[type=file]")).forEach(function(inp){
      ...
      if(acc.indexOf("image")>=0&&acc.indexOf("video")<0){inp.disabled=true;inp.style.display="none";}
...
function blockHkOpen(){
  ...
  var r=openR();
  if(r&&locked(r)){
    window.__lockSkip=true;roomId=null;
    try{if(typeof tab!=="undefined")tab="rooms";}catch(e){}
    alert("Room "+r.number+" is certified and locked.");
    try{draw();}catch(e){}
...
if(typeof viewRoom==="function"){
  ...
  viewRoom=function(){
    var r=openR();
    if(isHk()&&r&&locked(r))return "<p id=back>&larr; Back</p><div class=warn>Room "+r.number+" is certified and locked.</div>";
...
if(typeof bind==="function"){var _b=bind;bind=function(){try{_b.apply(this,arguments);}catch(e){}hideExtraMedia();restrictFdSelect();blockHkOpen();};}
if(typeof draw==="function"){var _d=draw;draw=function(){var out=_d.apply(this,arguments);hideExtraMedia();restrictFdSelect();blockHkOpen();return out;};}
try{draw();}catch(e){}
}
boot();
})();
