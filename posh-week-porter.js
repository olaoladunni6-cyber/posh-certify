(function(){
  ...
  window.viewDesk=function(){
    ...
    h+="<div class=card><h2>Porter check-in</h2><p>Front desk records payment. Porter walks the guest in.</p>"+
      "<input id=pName placeholder='Guest name'><select id=pRoom>"+vacant.map(function(r){return "<option>"+r.number+"</option>"}).join("")+"</select>"+
      "<input id=pOut type=date><button type=button class=btn id=porterIn>Check guest in</button></div>";
    h+="<div class=card><h2>After check-out inspection</h2><p>Look for damage and forgotten items before housekeeping.</p>";
    h+=inspect.map(function(r){
      return "<div class=card><b>Rm "+r.number+"</b> status "+r.status+"<input class=pDmg data-id='"+r.id+"' placeholder='Damage found'><input class=pForgot data-id='"+r.id+"' placeholder='Forgotten items'><button type=button class='btn pInsp' data-id='"+r.id+"'>Inspection done</button> <button type=button class='btn pFix' data-id='"+r.id+"'>Report fault</button></div>";
  ...
  window.bind=function(){
    ...
    if(pin) pin.onclick=function(){
      ...
      DB.checkins.push({id:"ci"+Date.now(),guest:name,room:num,amount:0,checkout:out,site:USER.site,by:USER.name,day:today(),kind:"stay",checkedOut:false,porter:USER.name});
      r.status="occupied"; r.guest=name; r.checkOut=out;
      if(typeof note==="function") note("Porter check-in "+name+" Rm "+num);
      save(); alert("Guest checked in Rm "+num); draw();
    };
    document.querySelectorAll(".pInsp").forEach(function(b){
      b.onclick=function(){
        var r=findRoom(b.getAttribute("data-id")); if(!r) return;
        var dmg=(document.querySelector(".pDmg[data-id='"+r.id+"']")||{}).value||"";
        var fg=(document.querySelector(".pForgot[data-id='"+r.id+"']")||{}).value||"none";
        DB.inspects=DB.inspects||[];
        DB.inspects.push({room:r.number,damage:dmg,forgotten:fg,by:USER.name,site:USER.site,at:now(),day:today()});
        if(dmg){
          DB.issues=DB.issues||[];
        ...
        r.status="pending"; r.guest=""; r.hk=""; r.hkName=""; r.videoReady=false; r.check=false;
        if(typeof note==="function") note("Porter inspect Rm "+r.number+" forgot "+fg);
        save(); alert("Inspection saved. Housekeeping can clean."); draw();
      };
    });
    document.querySelectorAll(".pFix").forEach(function(b){
      b.onclick=function(){
        ...
        if(typeof note==="function") note("Porter fault Rm "+r.number+" "+f);
